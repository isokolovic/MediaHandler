...
# Main method
def run_media_migration(source_folder, sort = 0):    
        ...
        for file in files:
            if (is_file_type_valid(file)):
                ...
                if(sort == 0):
                    ...
                    # a = WindowsAttributes(file)

                    year = datetime.datetime.fromtimestamp(os.path.getctime(file)).strftime("%Y")

                    if not os.path.exists(os.path.join(root, year)):
                        temp_dest_folder = os.mkdir(os.path.join(root, year))

                    temp_dest_folder = os.path.join(root, year)

                    shutil.move(os.path.join(root, file), os.path.join(temp_dest_folder, file))
...
if __name__ == "__main__":
    #0 = compress and move to another folder
    #1 = move according to date created
    #2 = move on remote location according to local folder structure
    run_media_migration(source_folder, 1)